...
const ShowTableByID = () => {
  ...
  useEffect(() => {
    ...
    if (
      Number.isInteger(valuePeopleAmount) &&
      Number.isInteger(valueMaxPeopleAmount)
    ) {
          ...
          <Form.Group as={Row} className='mt-4 mb-4'>
            ...
            {errorAmount && (
              <small className='d-block form-text text-danger mt-2'>
                Please insert correct value (0-10)
              </small>
            )}
          </Form.Group>
...
// //1.super wazna sprawa
// // table jest nullem ablo undefidem to : (ktos zle wpisal dane)
// // przekierowuje na homePage

/// 2.zastanow sie jakie dane sa potrzeba i dlaczego ???? najpierw sprawdz w console.log czy dziala i dopiero dispatczuj do stora
//co jak bedzie blad ? ? ?

///3. wyluskaj pojedynczy stolik useSelectorem Opis w instrukcjio

// //4. potrzebny bedzie dispatch i navigacja
// const dispatch = useDispatch();
// const navigate = useNavigate();

//5.najpierw sobie rospisz strukture a pozniej logike

// //6. // klasa styles.hidden bedzie aktywna jesli status !== 'Busy' jest true !
// // jesli status inny niz busy to element niewyswietli sie !
// .hidden {
//   display: none;
// }
